...
class AuthController extends Zend_Controller_Action
{
    ...
    public function ingresarAction()
    {
        ...
        Zend_Loader::loadClass('Zend_Auth_Adapter_DbTable');        //Cargamos la clase  de Zend Auth
            ...
            $authAdapter->setIdentity($email);                        //registramos el username enviado para su validacion
            ...
            if ($result->isValid()) {
                ...
                if ($data->usu_estado_id==1) {
                    $auth->getStorage()->write($data);
                    $response['valid'] = true;
                    $response['icon'] = 'success';
                    $response['mensaje'] = 'Parametros correctos!';
                    $response['ruta'] = $this->ruta_usuario($data->usu_id);
                } else {
                    $response['valid'] = false;
                    $response['icon'] = 'info';
                    $response['mensaje'] = 'El usuario no esta habilitado, comuniquese con el administrador';
                }
            } else {
                $response['valid'] = false;
                $response['icon'] = 'error';
                //Si las credenciales no son validas mostramos un error
                switch ($result->getCode()) {
                    case Zend_Auth_Result::FAILURE_IDENTITY_NOT_FOUND:
                        // usuario inexistente
                        $response['mensaje'] = 'El usuario que intenta acceder no existe!';
                        break;
                    case Zend_Auth_Result::FAILURE_CREDENTIAL_INVALID:
                        // password erroneo
                        $response['mensaje'] = 'La contraseña ingresada es incorrecta!';
    ...
    public function ruta_usuario($usu_id)
    {
        ...
        return $perfil->perf_controlador .'/'. $perfil->perf_accion;  
