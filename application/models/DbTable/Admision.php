...
class Application_Model_DbTable_Admision extends Zend_Db_Table_Abstract
{
    ...
    public function admisionpaciente(
        ...
        p_referido, p_contacto,p_forma_lleg_id, p_fuente_inf, p_quien_entrega,usu_id,nuhc,p_observacion)
        ...
        ARRAY['" . $institucion . "','" . $institucion_telefono . "'],".$usuario.",'".$nuhc."','".$observacion."');";
    ...
    public function edicionPaciente(
        ...
        $select="UPDATE paciente
                ...
                p_quien_entrega=ARRAY['" . $institucion . "','" . $institucion_telefono . "'], usu_id=".$usuario->usu_id.",nuhc='".$nuhc."', p_observacion='".$observacion."'
                WHERE p_ci='" . $cedula . "';";
        return $db->fetchRow($select);
    }
    /**
    ...
    public function listarPacientesCamaEspecialidad($especialidad)
    {
        //devuelve todos los registros de la tabla
        $db = Zend_Registry::get('pgdb');
        //opcional, esto es para que devuelva los resultados como objetos $row->campo
     ...
     * * esta funcion lista los pacientes que tienen una cama asignada de la bdd
     */
    public function listarPacientesCamaSala()
    {
        //devuelve todos los registros de la tabla
        $db = Zend_Registry::get('pgdb');
        //opcional, esto es para que devuelva los resultados como objetos $row->campo
        $db->setFetchMode(Zend_Db::FETCH_OBJ);
        $select = "SELECT e.especialidad_nombre,h.habitacion_nombre,c.cama_nombre,t.cama_tipo_descripcion,
	cp.fecha_ingreso,cp.p_id,cp.paciente_ci,cp.entrada
            FROM cama_paciente cp
            JOIN cama c
            ON c.cama_id=cp.cama_id
            JOIN habitacion h
            ...
            WHERE (cp.causa_id=1 OR cp.causa_id=6 OR cp.causa_id=7);";
        return $db->fetchAll($select);
    }
    /**
...
and nuhc not in (select paciente_ci from cama_paciente)";
        return $db->fetchAll($select);
    }
    /**
     * listarPacientes_CE
    ...
    public function updateCamaPaciente(
        $cama_paciente_id,
        $opcionCausa,
        $cama_id,
        $cie10_cod,
